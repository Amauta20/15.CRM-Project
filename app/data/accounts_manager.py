...
def add_account(name, company=None, email=None, phone=None, address=None, notes=None, user_role="", classification="Lead", status="Por contactar"):
    ...
    cursor.execute(
        "INSERT INTO accounts (name, company, email, phone, address, notes, classification, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
        (name, company, email, phone, address, notes, classification, status, now, now)
    )
    conn.commit()
...
def update_account(account_id, name, company=None, email=None, phone=None, address=None, notes=None, classification=None, status=None, user_role=""): # Added user_role
    if user_role != "Comercial":
    ...
    now = datetime.now().isoformat()
    conn.execute(
        "UPDATE accounts SET name = ?, company = ?, email = ?, phone = ?, address = ?, notes = ?, classification = ?, status = ?, updated_at = ? WHERE id = ?",
        (name, company, email, phone, address, notes, classification, status, now, account_id)
    )
    conn.commit()
    conn.close()
